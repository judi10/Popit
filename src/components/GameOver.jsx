...
export default function GameOver({ state, onReplay, onMenu, getBest }) {
  ...
  const msg = score === 0   ? 'Essaie encore !'
    ...
    : score < 160           ? 'Bien joué !'
    : score < 280           ? 'Excellent ! Tu as l\'oeil.'
    :                         'Champion !';
  ...
  return (
    <div style={s.screen}>
      <style>{`
        ...
        @media(prefers-reduced-motion:reduce){ .conf{ animation:none!important } }
      `}</style>

      {lives > 0 && (
        <div style={s.confettiWrap} aria-hidden="true">
          {confetti.current.map(c => (
            <div key={c.id} className="conf" style={{ position: 'absolute', top: -10, left: `${c.x}%`, width: c.size, height: c.size, borderRadius: 2, background: c.color, animation: `cFall 1.8s ease-in ${c.delay}s forwards` }} />
          ))}
        </div>
      )}
      ...
      {isNewBest && <div style={s.badge}>Nouveau record !</div>}
      ...
      <div style={s.card}>
        ...
        <div style={s.divider} />
        <div style={s.stats}>
          <Stat label="Niveau" value={`${lvl.emoji} ${lvl.label}`} />
...
const s = {
  screen:      { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '40px 24px', minHeight: '100dvh', textAlign: 'center', position: 'relative', overflow: 'hidden', background: '#0D0D1A' },
  confettiWrap:{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 },
  title:       { fontSize: 38, fontWeight: 900, position: 'relative', zIndex: 1, margin: 0 },
  ...
  replayBtn:   { padding: '15px 0', borderRadius: 50, border: 'none', background: 'linear-gradient(135deg,#7c3aed,#2563eb)', color: '#fff', fontSize: 17, fontWeight: 800, cursor: 'pointer', fontFamily: 'Nunito,sans-serif' },
  menuBtn:     { padding: '12px 0', borderRadius: 50, border: '2px solid #2a2a50', background: 'transparent', color: '#8b8ba8', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'Nunito,sans-serif' },
};